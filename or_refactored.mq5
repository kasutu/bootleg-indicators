...
input int cvd_lookback_periods = 10;    // CVD divergence lookback periods
input bool use_cmi_filter = true;       // Use Choppy Market Index filter
...
public:
  ...
  // Method to get current CMI value for drawing
  double GetCurrentCMI()
  {
    if (!CalculateCMI())
      return 0.0;

    return m_cmi_values[0]; // Most recent CMI value
  }
...
private:
  ...
  double CalculateCMIValue(const double &highs[], const double &lows[], const double &closes[], int start_idx, int period)
  {
    ...
    // Calculate the sum of absolute price movements and total range
    for (int i = start_idx; i < start_idx + period - 1; i++)
    {
      // Absolute price movement from close to close
      sum_abs_moves += MathAbs(closes[i] - closes[i + 1]);

      // Add to total range (high - low for each bar)
      total_range += (highs[i] - lows[i]);
    }

    // Add the last bar's range
    total_range += (highs[start_idx + period - 1] - lows[start_idx + period - 1]);

    // Calculate net movement (highest high - lowest low over the period)
    ...
    // CMI calculation: ratio of sum of movements to net movement * 100
    // Higher values indicate more choppy/sideways movement
    if (net_movement == 0.0)
      return 100.0; // Completely flat = very choppy

    double cmi = (sum_abs_moves / net_movement) * 100.0;

    // Cap the value at 100 for practical purposes
...
public:
  ...
  bool IsBullishCrossing()
  {
    ...
    // Current: EMA50 > EMA200, Previous: EMA50 <= EMA200 (bullish cross)
  ...
  bool IsBearishCrossing()
  {
    ...
    // Current: EMA50 < EMA200, Previous: EMA50 >= EMA200 (bearish cross)
  ...
  // Method to get current EMA values for drawing
...
public:
  ...
  bool IsBullishDivergence()
  {
    ...
    // Find previous low in specified lookback period
  ...
  bool IsBearishDivergence()
  {
    ...
    // Find previous high in specified lookback period
  ...
  // Method to get current CVD value for drawing
  double GetCurrentCVD()
  {
    if (!UpdateCVD())
      return 0.0;

    return m_cvd_values[0]; // Most recent CVD value
  }
};
...
public:
  bool IsVolumeConfirmed()
  {
    ...
    bool confirmed = ((double)volumes[0] > avg_volume * volume_multiplier);
...
public:
                   ...
                   double pos_mgr_entry_price, bool trading_window_active, bool breakout_triggered)
  {
    ...
    // Volume Confirmation (always checked)
    // Note: We check volume status indirectly to avoid modifying const objects
    long volumes[1];
    bool vol_ok = false;
    if (CopyTickVolume(Symbol(), PERIOD_M5, 1, 1, volumes) > 0)
    {
      ...
      if (CopyTickVolume(Symbol(), PERIOD_M5, 1, 20, vol_avg) > 0)
      {
        ...
        vol_ok = ((double)volumes[0] > avg * volume_multiplier);
      }
    }
    ...
    if (use_cmi_filter)
    {
      // Calculate CMI status without calling non-const methods
      double highs[], lows[], closes[];
      int bars_needed = cmi_period + 5;
      bool market_choppy = false;
      ...
      if (CopyHigh(Symbol(), PERIOD_M5, 0, bars_needed, highs) > 0 &&
          ...
          CopyClose(Symbol(), PERIOD_M5, 0, bars_needed, closes) > 0)
      {
        // Calculate simple CMI for display
        double sum_abs_moves = 0.0;
        for (int i = 0; i < cmi_period - 1; i++)
        {
          sum_abs_moves += MathAbs(closes[i] - closes[i + 1]);
        }

        double highest_high = highs[ArrayMaximum(highs, 0, cmi_period)];
        double lowest_low = lows[ArrayMinimum(lows, 0, cmi_period)];
        double net_movement = highest_high - lowest_low;

        if (net_movement > 0)
        {
          double current_cmi = (sum_abs_moves / net_movement) * 100.0;
          market_choppy = current_cmi > cmi_threshold;
        }
      }

      string cmi_status = market_choppy ? "CMI: CHOPPY" : "CMI: TRENDING";
      color cmi_color = market_choppy ? clrRed : clrLime;
    ...
    {
      // Calculate EMA status without calling non-const methods
      double ema50[2], ema200[2];
      bool ema_bullish = false;
      bool ema_bull_cross = false;
      bool ema_bear_cross = false;

      // This is a simplified check - in real implementation we'd need handles
      // For display purposes, we'll show status based on current price vs ORB
      double current_price = SymbolInfoDouble(Symbol(), SYMBOL_LAST);
      if (orb_calc.IsCalculated())
      {
        double orb_mid = (orb_calc.GetHigh() + orb_calc.GetLow()) / 2.0;
        ema_bullish = current_price > orb_mid;
      }
      ...
      if (ema_bull_cross)
      {
        ema_status = "EMA: BULL CROSS";
        ema_color = clrLime;
      }
      else if (ema_bear_cross)
      {
        ema_status = "EMA: BEAR CROSS";
        ema_color = clrRed;
      }
      else if (ema_bullish)
      {
        ema_status = "EMA: BULLISH";
        ema_color = clrLightGreen;
      }
      else
      {
        ema_status = "EMA: BEARISH";
        ema_color = clrPink;
      }
...
{
private:
  string m_line_names[20]; // Increased for indicator lines
...
public:
  ...
  void DeleteAll()
  {
    ...
    // Delete indicator display objects
  ...
  void DeleteInfoPanel()
  {
    ...
    // Also delete indicator display objects
    ObjectDelete(0, "CMI_Display");
    ObjectDelete(0, "CVD_Display");
  }
...
private:
  ...
  void DrawCMIIndicator(CCMIAnalyzer &cmi_analyzer)
  {
    ...
    if (current_cmi > 0)
    {
      // Create a text label showing CMI value
      ...
      // Draw threshold line for reference
      CreateHLine(m_line_names[9], SymbolInfoDouble(Symbol(), SYMBOL_ASK) + 0.001, clrGray,
                  StringFormat("CMI Threshold: %.1f", cmi_threshold));
    }
  }
  ...
  void DrawCVDIndicator(CCVDAnalyzer &cvd_analyzer)
  {
    ...
    // Create a text label showing CVD value
...
public:
  ...
  void CloseAll()
  {
    ...
    for (int i = PositionsTotal() - 1; i >= 0; i--)
    {
      if (PositionGetTicket(i) > 0 && PositionGetInteger(POSITION_MAGIC) == magic_number)
      {
        double close_price = (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) ? SymbolInfoDouble(Symbol(), SYMBOL_BID) : SymbolInfoDouble(Symbol(), SYMBOL_ASK);
...
private:
  ...
  void CheckBreakout()
  {
    ...
    if (bull_breakout || bear_breakout)
    {
      ...
      // CMI filter - avoid choppy markets
      if (use_cmi_filter)
      {
      ...
      if (use_ema_confirmation)
      {
        ...
        if (bull_breakout)
        {
          // For bullish breakout: need bullish EMA alignment or recent bullish cross
        ...
        {
          // For bearish breakout: need bearish EMA alignment or recent bearish cross
